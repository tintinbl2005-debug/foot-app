...
import { Brain, Target, Activity, ChevronLeft, Sparkles, Users, Flame } from "lucide-react";
...
function MatchDashboard() {
  ...
  // Dans ton composant MatchDashboard
useEffect(() => {
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white font-sans selection:bg-[#c1ff72] selection:text-black">
      ...
      <main className="mx-auto max-w-7xl px-4 py-10 space-y-10">
        <Link to="/" className="inline-flex items-center gap-2 text-white/50 hover:text-[#c1ff72] font-medium transition-colors">
        ...
        {isLoading ? (
          ...
          <div className="text-center py-20 bg-white/5 rounded-[2.5rem] border border-red-500/20">
            <h2 className="text-2xl font-bold text-red-400">Analyse indisponible</h2>
          </div>
        ) : (
          <div className="space-y-10 animate-in fade-in duration-700">
                        {/* 1. Scoreboard avec indicateur de genre */}
            <div className="relative overflow-hidden rounded-[3rem] border border-white/10 bg-white/5 backdrop-blur-xl p-8 sm:p-12 text-center grid grid-cols-3 items-center">
              ...
              <div className="flex flex-col items-center gap-3">
                <div className="grid h-24 w-24 place-items-center rounded-3xl bg-black/40 border border-white/10 p-4">
                  <img src={matchData.logo_home} alt="" className="h-full w-full object-contain" />
                </div>
              ...
              <div className="flex flex-col items-center gap-3">
                <div className="grid h-24 w-24 place-items-center rounded-3xl bg-black/40 border border-white/10 p-4">
                  <img src={matchData.logo_away} alt="" className="h-full w-full object-contain" />
                </div>
              ...
              {[ { team: matchData.team_home, prob: matchData.prob_home, odd: matchData.odd_home, edge: matchData.edge_home, isValue: matchData.value_bet_home },
                ...
                <div key={i} className={`p-6 rounded-[2rem] border ${bet.isValue ? "border-[#c1ff72] bg-[#c1ff72]/5 shadow-[0_0_20px_rgba(193,255,114,0.1)]" : "border-white/5 bg-white/5"}`}>
                  ...
                  <div className="mt-4 pt-4 border-t border-white/5 grid grid-cols-2 text-sm">
                    <div>Cote: <b className="text-white">{bet.odd}</b></div>
                    <div>Edge: <b className={bet.edge > 0 ? "text-[#c1ff72]" : "text-red-400"}>{bet.edge}%</b></div>
                  </div>
                </div>
              ))}
            </div>
            ...
            {/* 4. Rapport IA */}
            <div className="rounded-[2.5rem] border border-white/5 bg-white/5 p-6 backdrop-blur-sm">
              <h2 className="text-xl font-bold flex items-center gap-2 text-[#c1ff72] mb-6">
                <Brain className="h-5 w-5" /> Rapport Tactique Gemini
              </h2>
              <div className="text-white/70 whitespace-pre-line leading-relaxed text-base font-medium bg-black/30 p-6 rounded-2xl border border-white/5">
            ...
            {/* 5. Terrains */}
            <div className="rounded-[2.5rem] border border-white/5 bg-white/5 p-6 backdrop-blur-sm">
              <h2 className="text-xl font-bold flex items-center gap-2 text-white/80 mb-6">
                <Users className="h-5 w-5 text-[#c1ff72]" /> Alignements sur le Terrain
              </h2>
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                <div>
                  <h3 className="font-bold mb-4 flex justify-between bg-black/30 p-3 rounded-xl border border-white/5">
                    {matchData.team_home}
                  </h3>
...
function Pitch({ players, variant }: { players: any[], variant: "home" | "away" }) {
  return (
    <div className="relative aspect-[3/4] w-full overflow-hidden rounded-2xl ring-1 ring-white/10 shadow-2xl" style={{ background: "linear-gradient(180deg, #111c14 0%, #080d0a 100%)" }}>
      ...
      <svg viewBox="0 0 100 133" className="absolute inset-0 h-full w-full"><g fill="none" stroke="rgba(255,255,255,0.12)" strokeWidth="0.5"><rect x="2" y="2" width="96" height="129" /><line x1="2" y1="66.5" x2="98" y2="66.5" /><circle cx="50" cy="66.5" r="12" /><circle cx="50" cy="66.5" r="0.8" fill="rgba(255,255,255,0.2)" /><rect x="22" y="2" width="56" height="20" /><rect x="35" y="2" width="30" height="7" /><rect x="22" y="111" width="56" height="20" /><rect x="35" y="124" width="30" height="7" /></g></svg>
      {players.map((p) => (
        <div key={p.number} className="absolute -translate-x-1/2 -translate-y-1/2 transition-all duration-300" style={{ left: `${p.x}%`, top: `${p.y}%` }}>
          <div className="flex flex-col items-center gap-1">
            <div className={`grid h-8 w-8 place-items-center rounded-full text-xs font-black ring-2 ring-black/70 ${variant === "home" ? "bg-[#c1ff72] text-black shadow-[0_0_10px_rgba(193,255,114,0.3)]" : "bg-white text-black"}`}>{p.number}</div>
            <span className="rounded-md bg-black/80 border border-white/10 px-1.5 py-0.5 text-[9px] font-bold text-white max-w-[75px] truncate">{p.name.split(" ")[0]}</span>
          </div>
        </div>
      ))}
    </div>
