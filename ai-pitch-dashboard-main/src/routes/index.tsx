...
function LandingPage() {
  ...
  const handleGenerate = () => {
    ...
    if (matchFound) {
      navigate({ to: `/match/${matchFound.match_id}` });
    } else {
    ...
    <div className="min-h-screen bg-[#0a0a0a] text-white font-sans selection:bg-[#c1ff72] selection:text-black">
      ...
      <main className="flex flex-col items-center pt-16 px-4 pb-20 mx-auto max-w-7xl">
        ...
        <div className="w-full max-w-5xl space-y-8 pb-20">
          ...
          {isLoading ? (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5"><Skeleton className="h-28 rounded-[2rem] bg-white/5" /><Skeleton className="h-28 rounded-[2rem] bg-white/5" /><Skeleton className="h-28 rounded-[2rem] bg-white/5" /></div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
              {matches.map((m) => (
                <button key={m.match_id} onClick={() => navigate({ to: `/match/${m.match_id}` })} className="group flex justify-between p-5 rounded-[2rem] border border-white/5 bg-white/5 hover:border-[#c1ff72]/50 text-left">
                  <div className="flex flex-col gap-3">
                    <div className="flex items-center gap-4"><img src={m.logo_home} alt="" className="w-5 h-5" /><span className="font-semibold">{m.team_home}</span></div>
                    <div className="flex items-center gap-4"><img src={m.logo_away} alt="" className="w-5 h-5" /><span className="font-semibold">{m.team_away}</span></div>
